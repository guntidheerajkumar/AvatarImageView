...
NSString * const RoundAvatar_identifier = @"RoundAvatar.imagecache.tg";
...
@implementation RoundAvatarImageView
...
{
    return [[RoundAvatarImageView alloc] initWithFrame:frame
                      backgroundProgressColor:[UIColor whiteColor]
                                progressColor:[UIColor blueColor]];
}
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(RoundAvatarImageViewDidTapped:)])
    {
        [self.delegate RoundAvatarImageViewDidTapped:self];
    }
}
...
- (instancetype)init
{
    ...
    if(self)
    {
        ...
        self.cachePath      = [rootCachePath stringByAppendingPathComponent:RoundAvatar_identifier];
        
        if(![self.fileManager fileExistsAtPath:RoundAvatar_identifier])
        {
